...
cleaned_md_grants_loans <- grants_loans |>
  clean_names() |> 
...
pg_schools <- cleaned_payments |>
  filter(vendor_name == 'PRINCE GEORGES PUBLIC SCHOOLS' | vendor_name == 'PRINCE GEORGES COUNTY PUBLIC SCHOOLS' | vendor_name == 'PG COUNTY PUBLIC SCHOOL' | vendor_name == 'PRINCE GEORGES COUNTY PUBLIC SCHOOL' | vendor_name == 'PRINCE GEORGES COUNTY SCHOOL')
...
**A2: 2021 had the largest total amount, at 1,470,133,533.** **Spending seems to have increased in recent years (2019, 2020, 2021) in comparison to the earlier years in the dataset (2008, 2009, 2013.) I would wonder why there has been this increase.**
...
**A3. Right off the bat, I see a couple of vendor names that are listed as separate that should be listed as one vendor. If I wanted that to happen, I could use the case_when (str_detect) to standardize the name of each vendor. If I wanted to, I could mutate that data into a new column and group by that column instead.**

**Three vendor names that are similar and should be combined are:**

-   **A MORTON THOMAS & ASSOC INC vs. A MORTON THOMAS & ASSOCIATES**

-   **A&I INC. VS A&I INC,**

-   **AEROSOL MONITORING vs. AERSOL MONITORING AND ANALYSIS.**

```{r}
cleaned_payments |>
  filter(source == 'CAPITAL PROJECTS- UNIVERSITY OF MARYLAND') |>
...
**A4: It looks like stem cell research funding was the highest in the 2009 up to the early 2010s, but has decreased recently in the later 2010s and early 2020s.**
