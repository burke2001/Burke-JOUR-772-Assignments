...
```{r}
baltimore_county_ods <- read_csv("~/JOUR 772/Burke-JOUR-772-Assignments/labs/lab_05/tabula-Baltimore County; Carey, Samantha log OD.csv", col_names = FALSE) |> clean_names() |> 
  rename(date = x1, time = x2, case_number = x3, event_type = x4, location = x5) |>
  mutate(date = mdy(date))

View(baltimore_county_ods)
```
...
**A1. It looks like July 14 and October 4 are tied for the highest number of overdose calls in 2022, at 23 calls each. My result pulled 329 rows. Given that there are 365 days in a year, it would seem that 36 days in 2022 had no overdose calls.** **However, there could be a possibility that those days were simply just not included in the data set. I would need to call and verify.**

```{r}
baltimore_county_ods |>
...
**A2. It looks like Saturdays are the most common day of the week to receive overdose calls, at 638 total in the data set (15.52%).**
...
**A3. The location in the dataset that called most frequently was 4540 Silver Spring Road in Perry Hall, MD. From what I can tell on Google, it looks like a normal residence. However, 36 calls for one residence seems excessive. The second and third locations with the most calls, however, is a police department. This would make sense; I'm sure police officers come across a lot of overdoses while working.**

**One thing that makes the data potentially difficult is the fact that the location is not listed with a city; it could be possible that two locations have the same address and street name but are actually from two different cities/towns.**
...
**A4. The results for question 3 really left my scratching my head. I couldn't find any news articles on Google about 4540 Silver Spring Road at all, let alone any overdose articles...why would such a seemingly normal house be reporting overdoses so frequently? Could it be because the street corner they live next to is a frequent stop for those struggling with addiction? Or that they run a harm reduction facility from their house (which does not come up on Google)? I would love to do some digging on this, it really has me curious.**
