...
**Task** Run the following code and describe the dataframe it outputs. **Answer: A dataframe of 999 Ben Cardin press releases, along with the date, title, url, and text.**
...
# Telling R that for every item in the dataset, go through the webpage -> html: copy the source code -> know the exact path in that html file -> how many divs into the website to grab the text i want? save the text you see in the div we want
...
for (u in urls){
  ...
  html <- u |> read_html()
  # Collects the entire webpage that it links to
  ...
  text <- html |> 
    ...
    html_text()
# Grabs the contents of the press release and saves it as text
# Finding the amount of divs: go into source code and count the <div>s
...
#Go into the text category, replace this string of http, replace it with nothing instead. 
```
...
**Task** Run the following code and describe what the `unique` function does to the original list of words. **Answer: Lets R recognize these words as unique words.**
...
**Task** Run the following code and describe the output of using the `unnest_tokens` function. **Answer: Creates a data set in which every word in each press release now has its own row.**
...
**Task** Run the following code and describe what the resulting graphic shows. Is it interesting? **Answer: Creates a chart that shows the most-used words in Cardin press releases, ranked in descending order.**
...
**Task** Run the following code and describe the results. Is it more interesting than before? **Answer: These words are much more useful in analysis, now that the stop words have been removed. We now have more loaded, dynamic words.**
...
**Task** Run the following code and describe the results. **Answer: It gives us two tibbles of the top 10 words of 2022 and top 10 words of 2024.**
...
**Task** Run the following code and describe the results. **Answer: Shows the top 10 most common two-word phrases used in Cardin press releases for 2022. The top 3 are "amount included," "health care," and "Prince George's." Stop words are removed.**
...
**Task** Run the following code and describe the results. **Answer: Shows the most commonly used two-word phrases in Cardin press releases, with stop words removed, for 2024.**
...
**Task** Run the following code and describe the results. Do any of the sentiments seem incorrect or counter-intuitive? **Answer: Creates two new datasets analyzing words in the press releases, showing whether they have overall positive or negative connotations.**

**It looks like the sentiment measure for some words doesn't take into account multiple definitions of the same word. For some examples of potentially incorrect analyses in this context, I noticed that in the 2022 dataset, the word "issues" is listed as negative, when in a political context the term "issues" more likely refers to more neutral "points of discussion." In the 2024 dataset, the term "critical" is used as negative, when it is may also be being used as a synonym for "imperative."**
