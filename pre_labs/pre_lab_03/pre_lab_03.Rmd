...
**Task** Run the following codeblock to reload the data, using every row to set the data types. Does it show any parsing errors when you run? Answer below **Answer: No parsing errors.**
...
**Task** Run the following codeblock to glimpse the data. What data type is the "Vendor Name" field? What data type is the "Amount" field? What data type is the "Date" column? Answer below. **Answer: Amount is \<dbl\>, date is \<chr\>.**
...
**Task** Run the following codeblock to group by the vendor's zip code and show any that are less than five characters in length. How many zip codes do not have five characters? What do you think is happening with this data? **Answer: 8,458 zip codes in the data set have less than five characters.**
...
**Task** Run the following codeblock to count the number of payments. Looking at the result, what jumps out at you as potentially problematic? **Answer: There are N/A categories. In 2013, Loan was also separately abbreviated as L. There are no loans registered in 2009 or 2021.**
...
**Task** Run the following code to arrange the grants & loan data so that the smallest amount is first. Describe what you see. **Answer: In 2009, \$60.00 was given to Land Preservation Trust, Inc.**
...
**Task** Run the following codeblock to use the `clean_names()` function from janitor to standardize column names. How does the function change the headers? Answer below. **Answer: It lowercases and replaces spaces in headers with underscores.**
...
**Task** Run the following codeblock to check for duplicate rows using get_dupes(). How many duplicates are possible? **Answer: 50 duplicate rows**
...
**Task** Run the following codeblock to use distinct() to get rid of duplicate rows. How many rows does the new dataframe have? Answer below. **Answer: 17,740 rows**
...
**Task** Run the following codeblock to use str_sub() to convert the ZIP codes that have nine digits to five digits, creating a new column. Look at the difference in the result - what changed? **Answer: The column removed the -XXXX from nine-digit zip codes, to standardize zip codes to five digits.**
...
**The zip code receiving the most amount of money from the Tourism Board is 21202. This is the zip code for Baltimore. This makes sense, considering that Baltimore is probably Maryland's main tourist attraction.**
