...
In the space below, write the data type for the "id" column and the "seats" column (it's in light gray, offset by \< \>). **Answer: id is \<chr\>, seats is \<dbl\>**
...
-   **umd_courses: tells the computer to look at that data set**

-   **group by\_(term): tells computer to group the set by term**

-   **summarize ( count_classes =n() ): summarizes the data by number of classes per term**
...
**Answer: Arranges the class count in descending order.**
...
**Answer: Added the variable of departments in each term. The group_by(term, department) changed the output.**
...
**Answer: Counted the number of seats available in each term. The total_seats = sum(seats) function put this variable on the table.**
...
**Answer: Added columns for the mean number and median number of seats. Each of the top three terms have a mean number of seats in the 40s, and a median number in the 20s.**
...
1.  Are more classes available in the spring terms or fall terms?

2.  Are certain departments growing/shrinking over the years in terms of classes or seats available?

3.  Do upper-level courses have fewer seats available than lower-level courses?
